...
def main():
    ...
    if os.path.exists(str("D:\\nut\\code\\blendnet_win\\model\\miniimagenet\\mini_conv_blend_encoder_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")):
        feature_encoder.load_state_dict(torch.load(str("D:\\nut\\code\\blendnet_win\\model\\miniimagenet\\mini_conv_blend_encoder_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")))
        print("load feature encoder success")
    if os.path.exists(str("D:\\nut\\code\\blendnet_win\\model\\miniimagenet\\mini_conv_blendnet_"+ str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")):
    ...
    for episode in range(EPISODE):
        ...
        bc_f1, bc_f2, bc_f3, bc_f4 = feature_encoder(Variable(batches).cuda(GPU))

        smp_f1 = smp_f1.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 42,42)
        smp_f1 = torch.sum(smp_f1 ,1).squeeze(1)
        smp_f2 = smp_f2.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 21,21)
        smp_f2 = torch.sum(smp_f2 ,1).squeeze(1)
        smp_f3 = smp_f3.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 10, 10)
        smp_f3 = torch.sum(smp_f3 ,1).squeeze(1)
        smp_f4 = smp_f4.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 5, 5)
        smp_f4 = torch.sum(smp_f4 ,1).squeeze(1)
        ...
        bc_f1_ext = bc_f1.unsqueeze(0).repeat(CLASS_NUM,1,1,1,1)
        bc_f1_ext = torch.transpose(bc_f1_ext,0,1)
        bc_f2_ext = bc_f2.unsqueeze(0).repeat(CLASS_NUM,1,1,1,1)
        bc_f2_ext = torch.transpose(bc_f2_ext,0,1)
        bc_f3_ext = bc_f3.unsqueeze(0).repeat(CLASS_NUM,1,1,1,1)
        bc_f3_ext = torch.transpose(bc_f3_ext,0,1)
        bc_f4_ext = bc_f4.unsqueeze(0).repeat(CLASS_NUM,1,1,1,1)
        ...
        if episode%5000 == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                for test_images,test_labels in test_dataloader:
                    ...
                    smp_f4 = smp_f4.view(CLASS_NUM,SAMPLE_NUM_PER_CLASS,FEATURE_DIM,5,5)
                    smp_f4 = torch.sum(smp_f4,1).squeeze(1)
                    ts_f1, ts_f2, ts_f3, ts_f4 = feature_encoder(Variable(test_images).cuda(GPU))
